...
- (void)reset {
    ...
    self.transform = CGAffineTransformMakeRotation((arc4random_uniform(20) - 10) * M_PI / 180.0);
...
- (void)draggingEnded:(UIPanGestureRecognizer *)gesture{
    ...
    //if (self.dismissable && !CGRectContainsPoint(self.nonDismissingFrame, gesture.view.center)) { // view should be dismissed
        ...
        UIWindow *keyWindow = [[UIApplication sharedApplication] keyWindow];
        CGPoint diff = self.center;
        diff.x -=  keyWindow.center.x;
        diff.y -=  keyWindow.center.y;
        ...
        if (!self.gravityBehavior) {
            ...
            self.gravityBehavior.angle = atan2(diff.y, diff.x);
//            } else {
